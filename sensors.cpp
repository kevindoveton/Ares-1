...
//  Serial.println(String(offSetX) +" "+ String(offSetY) +" "+ String(offSetZ));
	compass.read();
...
RTVector3 Sensors :: readSensors() 
{
	...
	return RTVector3(pitch, roll, yaw);
...
void Sensors :: readGyro (RTVector3& gyr)
{
	...
	gyr.setX((gyro.g.x - offSetX) * GYRO_SCALE);
	gyr.setY((gyro.g.y - offSetY) * GYRO_SCALE);
	gyr.setZ((gyro.g.z - offSetZ) * GYRO_SCALE);
}

void Sensors :: AHRSupdate(float *dt) {
	static float gx;
	static float gy;
	static float gz;
	static float ax;
...
float Sensors :: fastAtan2( float y, float x)
{
	static float atan;
	static float z;
	if ( x == 0.0f )
...
float Sensors :: invSqrt(float number) {
	volatile long i;
	volatile float x, y;
...
void Sensors :: Smoothing(float *raw, float *smooth){
